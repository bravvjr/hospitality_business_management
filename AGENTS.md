...
**Phase 1 — Foundation + Inventory.** Completed in this repo: project structure, Docker/local
dev, Postgres + migrations, health endpoints, GitHub Actions CI, JWT auth + tenant context,
staff management, permissions (`require_permission`), staff account status, RLS,
sub-tenants, refresh-token rotation, rate limiting, and the inventory ledger module
(ADR-005: units, products, UoM, stock movements/levels).
**Next:** POS module.
